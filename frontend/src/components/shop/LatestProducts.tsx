// frontend/src/components/shop/LatestProducts.tsx
...
export const LatestProducts = ({ isHomePage = false }: LatestProductsProps) => {
    ...
    const fetchLatestProducts = async () => {
      try {
        const res = await fetch(
          `${process.env.NEXT_PUBLIC_API_URL}/api/products/latest?limit=8`
        );
        if (!res.ok) {
  ...
  return (
    <div className="mb-12">
      <h2 className="text-2xl font-bold mb-8">Najnowsze produkty</h2>
          ...
          <Link
            ...
            onClick={async (e) => {
              if (isHomePage) {
                ...
                try {
                  await trackEvent("home_latest_product_click", {
                    location: "home_page",
